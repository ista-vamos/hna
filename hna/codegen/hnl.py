...
class CodeGenCpp(CodeGen):
    ...
    def _generate_cache(self):
        with self.new_file("atoms-cache.h") as f:
            dump_codegen_position(f)
            for F, tmp in self._formula_to_automaton.items():
                num, _ = tmp
                f.write("std::map<")
                Q = F.trace_variables()
                f.write("Trace *,"*len(Q))
                f.write(f"Verdict> _verdicts_cache_atom_{num};")

        with self.new_file("cache-atom-result.h") as f:
            dump_codegen_position(f)
            w = f.write
            w('switch(atom_monitor->type()) {')

            for F, tmp in self._formula_to_automaton.items():
                num, _ = tmp
                w(f"""
                case ATOM_{num}: _verdicts_cache_atom_{num}[; break;
                """)
