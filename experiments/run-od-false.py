...
if len(sys.argv) > 1:
    ...
    functions = ['sampling.cpp']
    mondir = gen("forall t1, t2: (out(t1) <= out(samples(t1))) && (!(in(t1) <= in(t2)) || (out(t1) <= out(t2)))",
                 ",".join(alphabet), csv_header, functions)
...
def _run_measurement(NUM, LEN, VIOLATING, method):
    if method == "single-input":
        traces_dir = f"{mondir}/traces-{method}"
        gen_traces_single_input(alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING)
    elif method == "two-inputs":
        traces_dir = f"{mondir}/traces-{method}"
        gen_traces_two_inputs(alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING)
    elif method == "rand-inputs":
        traces_dir = f"{mondir}/traces-{method}"
        gen_traces_rand_inputs(alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING)
    else:
    ...
    for values in measure(inputs):
        W.writerow([NUM, LEN, VIOLATING] + [values[1], values[2]])
        sys.stdout.flush()
...
with open("output.csv", "a") as f:
    ...
    for method in ("single-input", "two-inputs", "rand-inputs"):
        ...
        for NUM in (50,):
            for LEN in (100,):
                times = _run_measurement(NUM, LEN, VIOLATING, method)
