...
if len(sys.argv) > 1:
    ...
    mondir = gen(
        "forall t1, t2: (out(t1) <= out(samples(t1))) && (!(in(t1) <= in(t2)) || (out(t1) <= out(t2)))",
        ",".join(alphabet),
        csv_header,
        functions,
    )
...
def _run_measurement(NUM, LEN, VIOLATING, method):
    if method == "single-input":
        ...
        gen_traces_single_input(
            alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING
        )
    elif method == "two-inputs":
        traces_dir = f"{mondir}/traces-{method}"
        gen_traces_two_inputs(
            alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING
        )
    elif method == "rand-inputs":
        traces_dir = f"{mondir}/traces-{method}"
        gen_traces_rand_inputs(
            alphabet, traces_dir, num=NUM, length=LEN, violating=VIOLATING
        )
    else:
